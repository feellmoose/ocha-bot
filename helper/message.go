...
var templates = map[string]map[string]string{
	...
	"cxg": {
		...
		"mine.game.rank.start.note":       "@{{ .Username }}\n喵喵喵~你的游戏开始啦～，nya！只要您这次扫雷挑战完成，成绩就会被记录到天梯赛榜单上哦~ 您已踏入全新 {{ .Width }} × {{ .Height }} 扫雷地图，埋伏了 {{ .Mines }} 颗地雷，nya~",
		"mine.game.rank.win.note":         "@{{ .Username }}\nNyaa~ 恭喜恭喜！🎉\n您只用了 {{ .Seconds }} 秒就通关了，真是太厉害了，nya！\n天梯赛得分\\排位：{{ .Score }}\\{{ .Rank }}，nya~\n地图尺寸：{{ .Width }} × {{ .Height }}，nya~\n地雷数量：{{ .Mines }}，nya~\n想查看完整榜单就用命令 \\mine_rank@{{ .BotName }}",
		"mine.game.rank.lose.note":        "@{{ .Username }}\n砰！💣\n好可惜，这次记录没能挤进天梯赛排位里，nya…\n耗时：{{ .Seconds }} 秒，nya~\n地图尺寸：{{ .Width }} × {{ .Height }}，nya~\n地雷数量：{{ .Mines }}，nya~\n要看详细榜单，请输入 \\mine_rank@{{ .BotName }}",
